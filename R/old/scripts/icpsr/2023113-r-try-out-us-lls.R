...
# provide: new_name = name of the new variable
# names_variables = the names of the variables that need to be recoded
# the value at which the counting should start

recode_and_rename_2 <- function(new_name, names_variables,start_value){
    x_columns <- grep("X", colnames(df.basic), value = TRUE)   # distill al X columns
  #select the first empty X column and add variables from old dataset here
  df.basic[,which(names(df.basic)%in%c(x_columns[1:(length(names_variables))]))]<-df[,c(which(names(df)%in%c(names_variables)))]
  col<-which(names(df.basic)%in%c(x_columns[1:(length(names_variables))]))
...
#test<-recode_and_rename_2("test", c("A_AGEDOB", "A_AGEDOBM", "A_AGEDOBD", "A_AGEDOBY", "A_ETHGRP"), 6)
#colnames(test)
#test$test7

# with flexible dataset
...
recode_and_rename_3 <- function(dataset, dataset_old, new_name, names_variables,start_value){
  dataset_new<-dataset
...
#test<-recode_and_rename_3(df.basic, df, "test", c("A_AGEDOB", "A_AGEDOBM", "A_AGEDOBD", "A_AGEDOBY", "A_ETHGRP"), 6)
#colnames(test)
#test$test8

# last change that still needs to be made: no number behind variable name if indicated
...
# + study id
# also make a file with excluded variables
...
df.basic<-recode_and_rename_3(df.basic,df,"g2v1c2inv",c("A_LVCURCH", "A_LVOTHCH", "A_LVOTHNB","NOCONTACT",
...
df.basic<-recode_and_rename_3(df.basic,df,"g3v1c3age",c("A_LVAGE1","A_LVAGE2","A_LVAGE3","A_LVAGE4","A_LVAGE5","A_LVAGE6","A_LVAGE7","A_LVAGE8","A_LVAGE9","A_LVAGE10","A_LVAGE11"),1)

# G2 relationship stability
# #NOTE.todo us-lls 22-11-2023  G2 relationship stability check which wave this is measured
df.basic<-recode_and_rename_3(df.basic,df,g2v1c2res, c("B_PSBGMO", "B_PSINF", "B_PSIMP" ),1)

# G2 romantic relationship quality
# #NOTE.todo us-lls 22-11-2023 G2 relationship quality check which wave this is measured

df.basic<-recode_and_rename_3(df.basic,df,"g2v1c2req", c("B_PSLIKE","B_PSACT","B_PSWRO","B_PSWROTIM","B_PSAFF","B_PSSUP",
...
df.basic<-recode_and_rename_3(df.basic,df,g2v1r2edu, c("C_PCSCHCM"),1)

# G2  education
df.basic<-recode_and_rename_3(df.basic,df,g2v1r2edu, c("W_SCHLVCOM"),1)

# G2  occupation
df.basic<-recode_and_rename_3(df.basic,df,g2v1r2occ, c("X_EMPCUR"),1)

# G2  occupation
df.basic<-recode_and_rename_3(df.basic,df,g2v1r2occ, c("X_EMPCUR"),1)

# G2 income
df.basic<-recode_and_rename_3(df.basic,df,g2v1vinc, c("Y_FINEARIN", "Y_FIN"),1)


# G2 partner work
# #NOTE.todo us-lls 22-11-2023 G2 partner work check which wave this is measured
df.basic<-recode_and_rename_3(df.basic,df,g2v1r2occ, c("C_PCEMP"),1)
...
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qupse", c("I_RECH_OPP", "I_RECH_NEG", "I_RECH_SHAM", "I_RECH_PROV",
                                                             "I_RECH_PRID"),1)

# pcs<-parent-child conflicts
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qupcs", c("I_CHSTR"),1)

# pcc<-parent-child closeness
# ppr<-praise/positive reinforcement
# paw<-positive affect/warmth 
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qupaw", c("H_PES_CHIL","H_PIS_CHIL", "I_CHENJOY", "I_CHSAT"),1)
...
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qupsy", c("Q__YELL","Q__RIDI","Q__EMB","Q__STUP","Q__SARC",
...
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qupun", c("Q_CHFM","Q_ROFM","Q_CHHR","Q_RMHR","Q_DKRM","Q_GRND",
...
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qucon", c("Q_IGNO","Q_REAS","Q_FIGNO1","Q_FIGNO2","Q_FIGNO3",
...
df.basic<-recode_and_rename_3(df.basic,df,"g2v1g2qupoth", c("Q_HUM","Q_TKDS","Q_TKTY","Q_TKPR","Q_FHUM1","Q_FHUM2",
...
# G2 alcohol use
df.basic<-recode_and_rename_3(df.basic,df,"g2v2c2qualc", c("M_SUBALC", "M_SUBALCYR","M_SUBALCYR1","M_SUBWKDRK","M_SUBWKNDR",
                                                           "M_SUBBINGE","M_SUBALCMO","M_SUBINMO","M_SUBDRUNK"),1)

# G2 drug use
df.basic<-recode_and_rename_3(df.basic,df,"g2v2c2qudru", c("M_SUBMJ","M_SUBMJYR","M_SUBMJTM","M_SUBMJAMT","M_SUBMJMO",
                                                           "M_SCR_CRWL", "M_SCR_INJUR", "M_SCR_NEEDL"),1)

# G2 substance use
df.basic<-recode_and_rename_3(df.basic,df,"g2v2c2qusub",c("M_SCRUSAL","M_SCR_TOMU","M_SCR_CUT","M_SCR_HELP","M_SCR_HLTH",
...
# option: make a file with excluded variables
...
colnames(df.excluded)
head(df.excluded$X424)
...
# Loop through the columns and pri
nt the attribute label
