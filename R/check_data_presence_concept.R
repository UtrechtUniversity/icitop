...
# Add a test row to rename_basic
rename_basic[90,"generation"] <- "g2"
rename_basic[90,"target"] <- "c"
rename_basic[90,"name_construct"] <- "age"
rename_basic[90,"wave"] <- "w2"

# # Old adding test rows
...
# TODO: add rows to docs/g1-moderators.xlsx where target is left empty
...
detect_missing_waves <- function(relevant_data_df){
  ...
  # TODO Dorien: make this data structure easier to read/unlist or something
  ...
  # FROM HERE ONWARDS: create table similar to missing_waves_df now including target
  #   construct missing_waves target
  #1       edu            w2  f
  #2       inc            w2  c
  #3       ses            w2  m
  
  # Regel: Als construct == par, dan moet er voor die wave waarin par gemeten 
...
detect_missing_waves(relevant_data_df)
